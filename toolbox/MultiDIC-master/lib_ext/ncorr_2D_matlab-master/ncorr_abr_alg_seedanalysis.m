function [seedinfo,convergence,outstate] = ncorr_abr_alg_seedanalysis(reference,current,roi,num_region,pos_seed,radius,cutoff_diffnorm,cutoff_iteration,enabled_stepanalysis,subsettrunc,num_img,total_imgs)
...
% all images will be seeded if outstate is set to success.
    ...
    for i = 0:length(current)-1
        ...
        disp(['Processing seed(s) for image ' num2str(num_img+i+1) ' of ' num2str(total_imgs) '...'])
        % ABR - END
    end        
    toc

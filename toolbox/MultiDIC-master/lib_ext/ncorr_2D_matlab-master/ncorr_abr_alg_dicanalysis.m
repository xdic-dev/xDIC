function [displacements,rois_dic,seedinfo,outstate] = ncorr_abr_alg_dicanalysis(imgs,radius,spacing,cutoff_diffnorm,cutoff_iteration,total_threads,enabled_stepanalysis,subsettrunc,num_img,total_imgs,pos_parent,params_init, initial_seed)
...
% outstate will only return failed if ncorr_alg_rgdic throws an exception.
    ...
    if (isempty(params_init))
        ...
        [seedinfo_prelim,threaddiagram,outstate_seeds] = ncorr_abr_gui_seedanalysis(imgs(1).imginfo, ...
                                                                                [imgs(2:end).imginfo], ...
                                                                                imgs(1).roi, ...
                                                                                radius, ...
                                                                                spacing, ...
    ...
    for i = 0:size(seedinfo_prelim,3)-1
        ...
        try
            ...
            % return an exception.
            seed_size =size(seedinfo_prelim); 
            nb_image = seed_size(3);
            disp(['Processing image ' num2str(i+1) ' of ', num2str(nb_image)]);

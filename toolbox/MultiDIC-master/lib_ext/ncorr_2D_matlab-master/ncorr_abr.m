...
%-------------------------------------------------------------------------%
    ...
    methods(Access = public)
        function obj = ncorr_abr(showGui, initial_seed)
        ...
        function set_dic_parameters_manually(obj, ...
                type, radius, spacing,cutoff_diffnorm,cutoff_iteration,...
                total_threads,stepanalysis_params)
                dispinfo_template.type = type;
                dispinfo_template.radius = radius; 
                dispinfo_template.spacing = spacing;
                dispinfo_template.cutoff_diffnorm = cutoff_diffnorm;
                dispinfo_template.cutoff_iteration = cutoff_iteration;
                dispinfo_template.total_threads = total_threads;
                dispinfo_template.stepanalysis = stepanalysis_params;
                dispinfo_template.subsettrunc = 0;                    
    ...
    methods(Access = public)
        ...
        function callback_topmenu_dic(obj,hObject,eventdata) %#ok<INUSD>
            ...
            function varargout = run_ncorr_abr_alg_dicanalysis(varargin)
                [varargout{1:nargout}] = ncorr_abr_alg_dicanalysis(varargin{:}, obj.initial_seed);
            end

            run_func = @run_ncorr_abr_alg_dicanalysis;
            if (obj.showGui)
                run_func = @ncorr_alg_dicanalysis;
            end
            ...
            if (strcmp(obj.data_dic.dispinfo.type,'regular'))  
                ...
                while (isempty(imgcorr_prelim) || imgcorr_prelim(end).idx_cur < length(imgs)-1)                    
                    ...
                    [displacements_buffer,rois_dic_buffer,seedinfo_buffer,outstate_dic] = run_func(imgs(imgcorr_prelim(end).idx_ref+1:imgcorr_prelim(end).idx_cur+1), ...
                ...
                while (isempty(imgcorr_prelim) || imgcorr_prelim(end).idx_cur > 0)                          
                    ...
                    % Returns failed if exception is thrown; for both 
                    % failed and cancel just return.
                    % ABR - EDIT FUNCTION CALL % 
                    [displacements_buffer,rois_dic_buffer,seedinfo_buffer,outstate_dic] = run_func(imgs(imgcorr_prelim(end).idx_ref+1:-1:imgcorr_prelim(end).idx_cur+1), ...
                                                                                                                obj.data_dic.dispinfo.radius, ...
                                                                                                                obj.data_dic.dispinfo.spacing, ...
                                                                                                                obj.data_dic.dispinfo.cutoff_diffnorm, ...
                                                                                                                obj.data_dic.dispinfo.cutoff_iteration, ...
        ...
        % This is the callback for formatting the displacements.
            ...
            if (strcmp(obj.data_dic.dispinfo.type,'regular'))
                ...
                run_func = @ncorr_abr_gui_formatdisp;
                if (obj.showGui)
                    run_func = @ncorr_gui_formatdisp;
                end

                % Format step displacements ------------------------------%
                [plots_disp_f_step_prelim,rois_f_step_prelim,pixtounits_prelim,units_prelim,cutoff_corrcoef_prelim,lenscoef_prelim,outstate_formatdisp_step] = run_func(imgs_ref, ...
                                                                                                                                                                                    imgs_cur, ...
        ...
        function handles_gui = init_gui(obj)
            ...
            handles_gui.topmenu_perfdic = uimenu( ...
                ...
                'Checked', 'off', ...
                'Callback', obj.util_wrapcallbackall(ncorr_util_wrapcallbacktrycatch( ...
                @(hObject,eventdata) callback_topmenu_dic(obj,hObject,eventdata),handles_gui.figure) ...
                ), ...
                'Enable', 'off', ...
                'Interruptible','off');
...
%   out.success.
    ...
    function constructor()
        ...
        % set(handles_gui_sethandle.figure,'Visible','off'); 
    end

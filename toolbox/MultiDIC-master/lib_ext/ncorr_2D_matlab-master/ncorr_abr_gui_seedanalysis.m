function [seedinfo,threaddiagram,outstate] = ncorr_abr_gui_seedanalysis(reference,current,roi,radius,spacing,cutoff_diffnorm,cutoff_iteration,total_threads,enabled_stepanalysis,subsettrunc,num_img,total_imgs,pos_parent, initial_seed)
...
% all images will be seeded if outstate is set to success. 
    ...
    function constructor()       
        ...
        if (true)
            ...
            set(handles_gui.figure,'Visible','off'); 
        end

        % ABR - trigger select region
        callback_button_selectregion()
    end
    ...
    function callback_button_selectregion(hObject,eventdata) %#ok<INUSD>
        ...
        while (~regionfinished)
            ...
            % Initialize point
            % ABR % handle_point = impoint(handles_gui.axes_selectregion);            
            
            % handle_point can be empty if escape key is pressed
            % ABR % if (~isempty(handle_point))          
            if (true)
                % Get position of point - convert to integer with zero based indexing
                % ABR % pos_region = round(getPosition(handle_point))-1;

                % Make sure point is within reduced ROI 
                % ABR % num_region_prelim = roi_reduced.get_num_region(pos_region(1),pos_region(2),list_region);  
                num_region_prelim = 0;

                if (num_region_prelim ~= -1)         
                    % Format point
                    % ABR % setColor(handle_point,'g');
                    % ABR % set(handle_point,'UIContextMenu','');
                    % ABR % set(handle_point,'ButtonDownFcn','');
                    ...
                    [seedinfo_buffer,threaddiagram_buffer,outstate_setseeds] = ncorr_abr_gui_setseeds(reference, ...
                                                                                                  current, ...
                                                                                                  roi, ...
                                                                                                  num_region_prelim, ...
                                                                                                  spacing, ...
        ...
        % ABR - AUTO TRIGGER FINISH
        callback_button_finish()
        % ABR - END
    end

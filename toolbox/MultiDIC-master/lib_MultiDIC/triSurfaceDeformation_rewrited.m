function [deformationStruct] = triSurfaceDeformation_rewrited(F,Vref,Vdef,varargin)
    ...
    for itri=1:nFaces
        if cum == 0 
            if itime == 1 
                ...
                % reference director vectors
                D1{itime}(itri,:)=Vdef{itime-1}(F(itri,2),:)-Vdef{itime-1}(F(itri,1),:);
                D2{itime}(itri,:)=Vdef{itime-1}(F(itri,3),:)-Vdef{itime-1}(F(itri,1),:);
                D3{itime}(itri,:)=cross(D1{itime}(itri,:),D2{itime}(itri,:))/norm(cross(D1{itime}(itri,:),D2{itime}(itri,:)));
            end
        else 
             % reference director vectors

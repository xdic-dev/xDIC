function [DIC3DStitched]= DIC3DsurfaceStitch(DIC3DAllPairsResults,pairIndList,varargin)
%% Parse input arguments
p = inputParser;
p.addRequired('DIC3DAllPairsResults'); 
p.addRequired('pairIndList'); 
p.addParameter('showvisu',0);
p.addParameter('title_ax',[]);
p.addParameter('pairforced',false);

p.parse(DIC3DAllPairsResults,pairIndList,varargin{:});
...
%         axisGeom
        ...
        [CT1,CT2] = removeOverlapSurface_temp_2018_10_30(F1,F2,V1,V2,[],[],minGap,pairforced);
...
% optStruct.supTitleString=title_ax;
if showvisu == 1
    anim8_rewrited(DIC3DStitched,'FacePairInds');
end

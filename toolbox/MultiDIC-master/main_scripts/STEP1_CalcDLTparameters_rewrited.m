...
%     NpNew=Np; % New number of points (1 column is deleted every loop)
%     PS=zeros(size(C)); % matrix for sorted points 
%     for icol = 1:NcCam(icount)
%         % find the lowest centroids in the image
%         [PtempYsort,PtempYsortInd]=sort(Ptemp(:,2),1,'descend');
%         PtempNow=Ptemp(PtempYsortInd(1:NcCam(icount)),:);
%         % select the left most centroid
%         [PtempXsort,PtempXsortInd]=sort(PtempNow(:,1),1,'ascend');
%         CentroidNow = PtempNow(PtempXsortInd(1),:);
% 
%         % select the centroid with the closest Y position
%         deltaX = Ptemp(:,1)-CentroidNow(1);
%         [Xtemp, XtempSortInd ]= sort(deltaX, 1, 'ascend');
%         colPoints = Ptemp(XtempSortInd(1:Nr),:);
% 
%         PS((icol-1)*Nr+1:icol*Nr,:) = sortrows(colPoints,2, 'descend');
%         Ptemp(XtempSortInd(1:Nr),:) = [];
%         NpNew = NpNew - Nr;
%     end
    ...
    PS=zeros(size(C)); % matrix for sorted points 
    for icol = 1:NcCam(icount)
        % find the left most column (smallest X)
        [PtempXsort,PtempXsortInd]=sort(Ptemp(:,1),1,'ascend');
        PtempXsortInd
        size(Ptemp)
        leftMostPoint = Ptemp(PtempXsortInd(1),:);

        % compute pairwise angular coefficient
        deltaP = Ptemp-leftMostPoint;
        angularCoefs = deltaP(:,2)./deltaP(:,1);

        % select the maximum in absolute value
        [leftMostAngularCoef, otherPointInd] = max(abs(angularCoefs));
        otherPoint = Ptemp(otherPointInd, :);

        % compute pairwise distance
        directionVector = otherPoint-leftMostPoint;
        directionVector = directionVector/norm(directionVector);
        pointDirectionVector = Ptemp-leftMostPoint;
           
        
        pointScalarProduct = abs(...
            pointDirectionVector(:,1)*directionVector(1) + ...
            pointDirectionVector(:,2)*directionVector(2) );

        pointDistance = pointDirectionVector(:,1).^2 + ...
            pointDirectionVector(:,2).^2;

        lineDistance = sqrt(abs(pointDistance - pointScalarProduct.^2));

        % select the points with the lowest distance to the line
        [linePoints,linePointsInd]=sort(lineDistance,1,'ascend');
        colPoints = Ptemp(linePointsInd(1:Nr),:);

        % add the points in order & remove from Ptemp
        PS((icol-1)*Nr+1:icol*Nr,:) = sortrows(colPoints,2, 'descend');
        Ptemp(linePointsInd(1:Nr),:) = [];
        NpNew = NpNew - Nr;
    end
...
for icam=indCams
    ...
    if saveDLTParametersLogic
        fullSavePath = [savePath filesep 'DLTstruct_cam_' num2str(icam)];
        disp(fullSavePath)
        save(fullSavePath,'DLTstructCam');
    end    

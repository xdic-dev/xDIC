function [seedinfo,threaddiagram,outstate] = ncorr_abr_gui_setseeds(reference,current,roi,num_region,spacing,radius,cutoff_diffnorm,cutoff_iteration,total_threads,enabled_stepanalysis,subsettrunc,num_img,total_imgs,pos_parent, initial_seed)
...
% if outstate returns true.
    ...
    scaled_initial_seed = initial_seed/(spacing+1);
    ...
    function callback_button_setseeds(hObject,eventdata) %#ok<INUSD>
        ...
        for i = 0:total_threads-1
            ...
            while (~seedfinished)   
                ...
                % ABR - REPLACE MANUAL SELECTION
                % handle_point_buffer = impoint(handles_gui.axes_setseeds);  
                if (first_run)
                    if (length(scaled_initial_seed)==2)
                        handle_point_buffer = impoint(handles_gui.axes_setseeds, ...
                            scaled_initial_seed);
                    else
                        roi_props = regionprops(roi_reduced.mask, 'Centroid');
                        handle_point_buffer = impoint(handles_gui.axes_setseeds, ...
                            roi_props.Centroid);
                    end
                end
        ...
        % ABR - AUTO TRIGGER CALLBACK FINISH
    ...
    function callback_button_finish(hObject,eventdata) %#ok<INUSD>
        ...
        if (size(unique(pos_seed,'rows'),1) == size(pos_seed,1))
            ...
            [seedinfo_prelim,outstate_preview] = ncorr_abr_gui_seedpreview(reference, ...
                                                                       current, ...
                                                                       roi, ...
                                                                       num_region, ...
                                                                       pos_seed, ...

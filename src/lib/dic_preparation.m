function [prep_params] = dic_preparation(base_data_path, base_result_path)
% DIC_PREPARATION Prepare data and parameters for DIC analysis
%   This function handles all initialization and data preparation for the DIC
%   analysis pipeline.
%
% Parameters:
...
base_parameters = struct(...
    ...
    'baseResultPath', base_result_path,...
    'subject', subject_id,...
...
if ~isempty(p.Results.reftrial_setmanual)
    reftrial = p.Results.reftrial_setmanual;
end
...
% Image filtering
if im_filter_mode
    [cam_first, gsboundaries] = filter_like_ben(cam_first_satur, 'paramfilt', p.Results.param_filt_im);
    cam_second = filter_like_ben(cam_second_satur, 'gsbound', gsboundaries, 'paramfilt', p.Results.param_filt_im);

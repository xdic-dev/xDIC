...
normals = zeros([size(points),3]);
curvature = zeros(size(points,1),3);
for ii = 1:(size(points,1))
    
    %form covariance matrix
    Cmat = [C(ii,1) C(ii,2) C(ii,3);...
        C(ii,2) C(ii,4) C(ii,5);...
        C(ii,3) C(ii,5) C(ii,6)];  

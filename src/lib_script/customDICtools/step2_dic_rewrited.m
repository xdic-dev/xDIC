...
p.addParameter('cam_first',[]);
p.addParameter('cam_second',[]);

p.parse(basePath, varargin{:});
cam_1 = p.Results.cam_1;
cam_2 = p.Results.cam_2;
cam_first = p.Results.cam_first;
cam_second = p.Results.cam_second;

%% data loading
ncorr1 = load(fullfile( basePath, "ncorr"+num2str(cam_1)+".mat"));
ncorr2 = load(fullfile( basePath, "ncorr"+num2str(cam_2)+".mat"));
ncorr1_2 = load(fullfile( basePath, "ncorr"+num2str(cam_1)+"_"+num2str(cam_2)+".mat"));
...
disp('loading Done');
%% filtering
% data1 = test_filtering3(data1); 
% data1_2 = test_filtering3(data1_2); 
% data2 = test_filtering3(data2); 
% disp('cleaning Done');

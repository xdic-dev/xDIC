...
FACEMEASURE = {'Epc1','Epc2','DispZ'};

% 
trial_target_set = search_trial2target(baseDataPath,SUBJECT,PHASE,MATERIAL_NBR,...
    NFCOND_SET,SPDDXLCOND_SET);

TRIAL_TARGET = trial_target_set(1:end);
...
mask_outliers_indiv_id = cell(Ntrial,2);
id_all = zeros(Ntrial,3); 
for mm = 1:Ntrial
    r = data_dic{mm};
    id = [1,idx_touch_all(mm)+10,length(r.FaceCentroids)];
    for iframeMap = 1:length(id)
        ptFC_cur = r.FaceCentroids{id(iframeMap)};
        ...
        mask_outliers_indiv_id{mm,iframeMap} = temp|isNotValid;
    end
    mask_comb_current = false(length(r.FaceCentroids{1}),1);
    for iframeMap = 1:length(id)
        mask_comb_current = mask_comb_current|mask_outliers_indiv_id{mm,iframeMap}; 
    end
    id_all(mm,:) = id; 
...
for mm = 1:Ntrial
    ...
    for iframeMap = 1:length(id)
        ptFC_cur = r.FaceCentroids{id(iframeMap)};
        mask_indiv_current = mask_outliers_indiv_id{mm,iframeMap}; 
...
for mm = 1:Ntrial
    ...
    Nframe = length(r.FaceCentroids);
    Fcentroidfirst=r.FaceCentroids{1};
    Fcentroidlast=r.FaceCentroids{end};
    ...
    corrLogic = r.FaceCorrComb{end} > optStruct.corr_threshold; 
    % mask stitch faces
    gapLogicStitch = r.FacePairInds==3;%|r.FacePairInds==1;% ATTENTION A CHANGER !!
    ...
    % time filtering 
    r = filter_measures(r,optStruct.freqTempFilt,actual_FS_vid,FACEMEASURE(1:length(FACEMEASURE)-1)); %last is curvature
        
    for is = 1:length(FACEMEASURE)-1
        faceMeasure_is = FACEMEASURE{is}; 
        
        %Load current measurements from DIC analysis
        [FCmat_cum,FCmat_rate] = data_dic2matrix(r,faceMeasure_is); 
        if strcmp(faceMeasure_is,'Area')
            A = FCmat_cum(:,1);
            [FCmat_cum,FCmat_rate] = data_dic2matrix(r,'J'); 
        ...
        elseif strcmp(faceMeasure_is,'Epc1')||strcmp(faceMeasure_is,'Epc2')
            ...
            FC_all.faceMeasure_is.vec = r.Deform.Epc1vecCur{Nframe};
        ...
        for iframe = 1:Nframe
            Fcentroidnow=r.FaceCentroids{iframe}; %Npoints x 3
    ...
    T{mm}.Nframe = Nframe; 
    T{mm}.nfcond = NFCOND_SET;
    T{mm}.spddxlcond = SPDDXLCOND_SET;
    T{mm}.FC = FC_all;
    T{mm}.nf = nf_all(:,mm); 
...
for mm = 1:Ntrial
    ...
    for iframe = id
        ...
        h_line = plot3(h_ax(ax_idx),zeros(length(xvec),1),xvec,y_est2,'k');
        
        title(h_ax(ax_idx),sprintf('trial #%d\nangle = %1.2f°',TRIAL_TARGET(mm),angle)); 
        if axis_target == 1
...
for mm = 1:Ntrial
    ...
    folder_loc = fullfile(baseResultPath,'analysis',SUBJECT,FRcond{MATERIAL_NBR},sprintf(sprintf("%03d",trial_cur))); 
    if ~exist(folder_loc,'dir')
        mkdir(folder_loc);
    end
    filename = sprintf('%s_%03d_proccessed_data_loading.mat',SUBJECT,trial_cur); 
    data = T{mm};
...
for iframeMap = 1:length(id)
    ...
    FC_epc1 = T{mm}.FC.Epc1.cum(:,id(iframeMap)); D1 = r.Deform.Epc1vecCur{id(iframeMap)};
    FC_epc2 = T{mm}.FC.Epc2.cum(:,id(iframeMap)); D2 = r.Deform.Epc2vecCur{id(iframeMap)};
    for xx = 1:Ncirc
        ax_idx = mm+(xx-1)*Ncol;
        ...
        for kk =1:Nrad
            ...
            pnow = mean(xyz,1,'omitnan');
            Ds1now = median(vec_region,1,'omitnan').*median(FC_epc1_region,1,'omitnan'); %scaled vector
            Ds2now = median(D2_region,1,'omitnan').*median(FC_epc2_region,1,'omitnan'); %scaled vector
            %plot principales directions projected onto 2D plane 
            h_quiv1(kk) = quiver(h_ax(ax_idx),pnow(1)-Ds1now(:,1)/2,pnow(2)-Ds1now(:,2)/2,Ds1now(:,1),Ds1now(:,2),0,...
                'Color',color_contraction,'ShowArrowHead','off','AutoScale','off','LineWidth',3);
            h_quiv2(kk) = quiver(h_ax(ax_idx),pnow(1)-Ds2now(:,1)/2,pnow(2)-Ds2now(:,2)/2,Ds2now(:,1),Ds2now(:,2),0,....
                'Color',color_elongation,'ShowArrowHead','off','AutoScale','off','LineWidth',3);
...
for iframeMap = 1:length(id)
    ...
    for kk =1:Nrad   
        ...
        for xx = 1:Ncirc
            ...
            Ds2now = median(D2_region,1,'omitnan').*median(FC_epc2_region,1,'omitnan'); %scaled vector
            %plot principales directions projected onto 2D plane 
            h_quiv1(xx) = quiver(h_ax(ax_idx),pnow(1)-Ds1now(:,1)/2,pnow(2)-Ds1now(:,2)/2,Ds1now(:,1),Ds1now(:,2),0,...
                'Color',color_contraction,'ShowArrowHead','off','AutoScale','off','LineWidth',3);
            h_quiv2(xx) = quiver(h_ax(ax_idx),pnow(1)-Ds2now(:,1)/2,pnow(2)-Ds2now(:,2)/2,Ds2now(:,1),Ds2now(:,2),0,...
                'Color',color_elongation,'ShowArrowHead','off','AutoScale','off','LineWidth',3);
...
for mm = 1:Ntrial
    ptFC_cur = T{mm}.FC.xyz(:,:,id(iframeMap)); %r.FaceCentroids{id(iframeMap)};
        ...
        %Get border of contact and reorder to have a nice shape 
        current_mask = mask_in_border_current;
        ca_border = ptFC_cur(current_mask,:);
        if ~isempty(ca_border) & length(ca_border)>3
%             ca_border = ca_border(~isnan(current_mask),:);
            k = boundary(ca_border(:,1),ca_border(:,2),0.2);
            ca_border = ca_border(k,:);
            ca_border_fitted = myfitellipse(ca_border(:,1),ca_border(:,2)); 
            ca_border_fitted = [ca_border_fitted,repelem(mean(ca_border(:,3),'omitnan'),length(ca_border_fitted),1)];
        else
            ca_border = [NaN,NaN,NaN]; 
            ca_border_fitted = [NaN,NaN,NaN]; 
        end
        ...
        % plot contour 
        plot3(h_ax(ax_idx),ca_border(:,1),ca_border(:,2),ca_border(:,3),'k-','LineWidth',2);
        % plot fitted ellipse 
        plot3(h_ax(ax_idx),ca_border_fitted(:,1),ca_border_fitted(:,2),ca_border_fitted(:,3),'-','Color',[0.8500 0.3250 0.0980],'LineWidth',3);
...
%% COMPUTATION - time dependent variables 
fprintf('Computation - time varying variables...'); 
Nframe_max_all = 0; 
for mm = 1:Ntrial
...
% Parameters
p_in = 50; %percent of point in contact 
Nrad = 4;
FC_rate_val_all = NaN(Nframe_max_all,Nface,Nrad,Ncirc,Ntrial); 
FC_cum_val_all = NaN(Nframe_max_all,Nface,Nrad,Ncirc,Ntrial); 
inside_logic_all = NaN(Nframe_max_all,Nface,Nrad,Ncirc,Ntrial); 
FCmat_cum_last_all = cell(1,Ntrial);

for mm = 1:Ntrial
    Nframe = T{mm}.Nframe; 
    Ndata = length(T{mm}.FC.(FACEMEASURE{1}).cum); 
    FCmat_cum_last_all_cur = NaN(Ndata,Nface*(Nrad),Ncirc); 
    mask_in_current = T{mm}.mask_in; 
    for iface = 1:Nface
        faceMeasure_ii = FACEMEASURE{iface}; 
        ...
        for xx = 1:Ncirc
            ...
            for kk = 1:Nrad
                ...
                if kk == 5 
                    ...
                    current_mask = mask_circ_current&mask_rad_current;
                end
                FC_cum_current(~current_mask) = NaN;
                FC_rate_current(~current_mask) = NaN;
                
                %find whether any point of the region is inside the contact
                %area. If 'p_in'% of the total number of point in the current region
                %is found inside, the region is considered in contact.
                NbrPointRegionTot = sum(~isnan(FC_cum_current(:,1)));
                NbrPointRegionCur = sum(current_mask&mask_in_current,1);
                inside_logic_current = NbrPointRegionCur>=NbrPointRegionTot*p_in*1e-2;
                ...
                %
                FC_rate_val_all(1:Nframe,iface,kk,xx,mm) = FC_rate_val_current; 
                FC_cum_val_all(1:Nframe,iface,kk,xx,mm) = FC_cum_val_current; 
                inside_logic_all(1:Nframe,iface,kk,xx,mm) = inside_logic_current; 
                %
                FCmat_cum_last_all_cur(:,kk+(iface-1)*Nrad,xx) = FC_cum_current(:,end); %all data at last iteration 
                
%                 %ax 1°
%                 %plot var vs time
%                 if (ax_idx < Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
%                 plot(h_ax(ax_idx),t(inside_logic_current), FC_rate_val_current(inside_logic_current),'Marker',marker_set{kk},'Color',color_set_cell{kk},'MarkerFaceColor',color_set_cell{kk},'MarkerSize',ms); 
%                 plot(h_ax(ax_idx),t(~inside_logic_current), FC_rate_val_current(~inside_logic_current),'Marker',marker_set{kk},'Color',color_set_cell{kk},'MarkerFaceColor','w','MarkerSize',ms); 
    ...
    %output var
    T{mm}.FC_rate_val = FC_rate_val_all(:,:,:,:,mm);
    T{mm}.FC_cum_val = FC_cum_val_all(:,:,:,:,mm);
    T{mm}.inside_logic = inside_logic_all(:,:,:,:,mm);
    T{mm}.FCmat_cum_last = FCmat_cum_last_all_cur;
end
...
%%
figure; hold on; 
hp{1} = plot(NaN,NaN,'k--','LineWidth',3); 
hp{2} = plot(NaN,NaN,'k-','LineWidth',3); 
hleg = legend([hp{:}],{'border in','border out'}); 
set(hleg,'FontSize',12,'box','off'); 
%% MAP TOTAL MEAN OVER EACH SEGMENTATION 
colorbarLimits = [-10,10];
% Figure and subplot initialization 
title_fig = sprintf("%s : Map total mean over each segmentation \n%s",SUBJECT,mat2str(TRIAL_TARGET)); 
hf = newfig(title_fig);
...
for iface = 1:2%Nface
    ax_idx = iface+(mm-1)*2;
    h_quiv = gobjects(1,Ncirc*Nrad);
    faceMeasure_ii = FACEMEASURE{iface};
    ptFC_cur = T{mm}.FC.xyz(:,:,Nframe); %r.FaceCentroids{id(iframeMap)};
    vec_cur = T{mm}.FC.(faceMeasure_ii).vec;
    scatter(h_ax(ax_idx),ptFC_cur(:,1),...
        ptFC_cur(:,2),...
        1,'Marker','.',...
        'MarkerEdgeColor',[1 1 1].*0.5,'MarkerFaceColor',[1 1 1].*0.5);
    ...
    for kk = 1:Nrad   
        mask_rad_current = T{mm}.mask_rad{kk};
        for xx = 1:Ncirc
            FC_cur = T{mm}.FC_cum_val(:,iface,kk,xx);
            mask_circ_current = T{mm}.mask_circ{xx};
            %Fill with NaN outside region
            current_mask = mask_rad_current(:,1)&mask_circ_current(:,1);
            xyz = ptFC_cur(current_mask,:);
            vec_region = vec_cur(current_mask,:);
            Nelemcur = sum(current_mask); 
            
            %centroid, average face value and direction
            pnow = mean(xyz,1,'omitnan'); %centroid
            facenow = FC_cur(end); %value
            vecnow = median(vec_region,1,'omitnan')*facenow; %scaled vector
            
            %plot X-Y points of current region 
            scatter(h_ax(ax_idx),xyz(:,1),xyz(:,2),5,repelem(facenow,Nelemcur,1),"filled");
            %plot principales directions projected onto 2D plane 
            h_quiv(xx+(kk-1)*Ncirc) = quiver(h_ax(ax_idx),pnow(1)-vecnow(:,1)/2,pnow(2)-vecnow(:,2)/2,vecnow(:,1),vecnow(:,2),0,...
                'Color','k','ShowArrowHead','off','AutoScale','off','LineWidth',3);
        end
    end
    caxis(h_ax(ax_idx),colorbarLimits);
    if mm == 1
        text(h_ax(ax_idx),mean(ptFC_cur(:,1),1,'omitnan'),min(ptFC_cur(:,2),[],1,'omitnan'),0,sprintf('%s',faceMeasure_ii),...
        'Rotation',0,...
        'VerticalAlignment','bottom',...
        'HorizontalAlignment','center',...
        'FontSize',15,...
    ...
    caxis(h_ax(ax_idx),[-10,10]);
    % plot contour 
    plot(h_ax(ax_idx),T{mm}.ca_last(:,1),T{mm}.ca_last(:,2),'k-','LineWidth',1,'Color',[1 1 1].*0.3);
    % set quiver plot to front of the axis.
    uistack(h_quiv,'top');
end
end
...
ms = 4; 
marker_set = 'o'; 
xlabel_set = 'Time (s)'; xlim_set = [0,t(end)];
ylabel_set = 'TOTAL (%)'; ylim_set = [-10,10];
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Radial time %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.0,0.0287,0.5,0.8926]);
Nface = length(FACEMEASURE);
Nrow = Ncirc; 
Ncol = length(FACEMEASURE); 
mergeElement = {};
h_ax = subplot_ax(Nrow,Ncol,'merge',mergeElement,'shownum',0); hold(h_ax,'on'); 
for iface = 1:Nface
    faceMeasure_ii = FACEMEASURE{iface};
    for xx = 1:Ncirc
        ...
        for kk = 1:Nrad
            inside_logic_current = logical(squeeze(inside_logic_all(:,iface,kk,xx,:)));
            FC_current = squeeze(FC_cum_val_all(:,iface,kk,xx,:));
            tmat = repmat(t,Ntrial,1)';
            inside_logic_current_mean = mean(inside_logic_current,2)>0.5;
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
%                 'nstd',nstd,...
%                 'FaceAlpha',0.5);
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            %Line plot
            plot(h_ax(ax_idx),tmat,FC_current,'Color',color_set_cell{kk},...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
            %Black dots - outside contact
            plot(h_ax(ax_idx),tmat(~inside_logic_current),FC_current(~inside_logic_current),'Color',color_set_cell{kk},...
                'LineStyle','none',...
                'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor','k')
            %Colored dots - inside contact
            plot(h_ax(ax_idx),tmat(inside_logic_current),FC_current(inside_logic_current),'Color',color_set_cell{kk},...
                'LineStyle','none',...
                'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',color_set_cell{kk});
            % title
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            % x-axis
            xlim(h_ax(ax_idx),xlim_set);
            if xx~=Ncirc; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if xx~=Ncirc; h_ax(ax_idx).XTickLabel = []; end
            if xx==Ncirc; xlabel(h_ax(ax_idx),xlabel_set); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylim_set); end
            if (iface==2); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface==2; h_ax(ax_idx).YTickLabel = []; end
            if (iface==1) && (xx==(floor(Ncirc/2)+1)); ylabel(h_ax(ax_idx),ylabel_set,'FontSize',25); end
            %
            if kk==Nrad
                xlimit = get(h_ax(ax_idx),'XLim');
                ylimit = get(h_ax(ax_idx),'YLim');  
                text(h_ax(ax_idx),mean(xlimit),ylimit(2),T{1}.name_circ{xx},...
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Circumferencial time %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.5,0.0287,0.5,0.8926]);
Nface = length(FACEMEASURE);
Nrow = Nrad; 
Ncol = length(FACEMEASURE); 
mergeElement = {};
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            %Line plot
            plot(h_ax(ax_idx),tmat,FC_current,'Color',color_set_cell{xx},...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
%             %Black dots - outside contact
%             plot(h_ax(ax_idx),tmat(~inside_logic_current),FC_current(~inside_logic_current),'Color',color_set_cell{xx},...
%                 'LineStyle','none',...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor','k')
%             %Colored dots - inside contact
%             plot(h_ax(ax_idx),tmat(inside_logic_current),FC_current(inside_logic_current),'Color',color_set_cell{xx},...
%                 'LineStyle','none',...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',color_set_cell{xx});
            %inside logic 
            if xx==round(Ncirc/2)
                x = t(mean(~inside_logic_current,2)>0.5); x = [xlim_set(1),x(end)];
                fill(h_ax(ax_idx),[x fliplr(x)],repelem(ylim_set,1,2),[1 1 1].*0.0,'LineStyle','none','FaceAlpha',0.1);
            end
            %Title
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            % x-axis
            xlim(h_ax(ax_idx),xlim_set);
            if kk~=Nrad; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if kk~=Nrad; h_ax(ax_idx).XTickLabel = []; end
            if kk==Nrad; xlabel(h_ax(ax_idx),xlabel_set); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylim_set); end
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
            if (iface==1) && (kk==(floor(Nrad/2)+1)); ylabel(h_ax(ax_idx),ylabel_set,'FontSize',25); end
            %
            if xx==Ncirc
                xlimit = get(h_ax(ax_idx),'XLim');
                ylimit = get(h_ax(ax_idx),'YLim');  
                text(h_ax(ax_idx),mean(xlimit),ylimit(2),T{1}.name_rad{kk},...
                    'FontSize',12,'Interpreter','none',...
                    'HorizontalAlignment','center','VerticalAlignment','middle');
            end
            %
...
ms = 4; 
marker_set = 'o'; 
xlabel_set = 'Force (N)'; xlim_set = [0,5];
ylabel_set = 'TOTAL (%)'; ylim_set = [-10,10];
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Radial Force %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);
...
for iface = 1:Nface
    ...
    for xx = 1:Ncirc
        ...
        for kk = 1:Nrad
            ...
            FC_current = squeeze(FC_cum_val_all(:,iface,kk,xx,:));
            x = nf_all;
            inside_logic_current_mean = mean(inside_logic_current,2)>0.95;
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
%                 'nstd',nstd,...
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            %Line plot
            plot(h_ax(ax_idx),x,FC_current,'Color',color_set_cell{kk},...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
            %Black dots - outside contact
            plot(h_ax(ax_idx),x(~inside_logic_current),FC_current(~inside_logic_current),'Color',color_set_cell{kk},...
                'LineStyle','none',...
                'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor','k')
            %Colored dots - inside contact
            plot(h_ax(ax_idx),x(inside_logic_current),FC_current(inside_logic_current),'Color',color_set_cell{kk},...
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Circumferencial Force %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            % 0 - baseline
            %Line plot
            plot(h_ax(ax_idx),x,FC_current,'Color',color_set_cell{xx},...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
%             %Black dots - outside contact
%             plot(h_ax(ax_idx),x(~inside_logic_current),FC_current(~inside_logic_current),'Color',color_set_cell{xx},...
%                 'LineStyle','none',...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor','k')
%             %Colored dots - inside contact
%             plot(h_ax(ax_idx),x(inside_logic_current),FC_current(inside_logic_current),'Color',color_set_cell{xx},...
...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',color_set_cell{xx});
            %inside logic 
            if xx==round(Ncirc/2)
                x = nf_all(mean(~inside_logic_current,2)>0.95,1); x = [xlim_set(1),max(x)];
                fill(h_ax(ax_idx),[x fliplr(x)],repelem(ylim_set,1,2),[1 1 1].*0.0,'LineStyle','none','FaceAlpha',0.1);
            end
            %Title
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            ...
            %
            if xx==Ncirc
                text(h_ax(ax_idx),mean(xlim_set),ylim_set(2),T{1}.name_rad{kk},...
                    'FontSize',12,'Interpreter','none',...
                    'HorizontalAlignment','center','VerticalAlignment','middle');
            end
            %
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Annulus time %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.0005    0.3944    0.3870    0.5269]);
Nface = 2; %length(faceMeasure);
Nrow = 1; 
Ncol = 2; %length(faceMeasure); 
mergeElement = {};
h_ax = subplot_ax(Nrow,Ncol,'merge',mergeElement,'shownum',0); hold(h_ax,'on'); 
...
for iface = 1:Nface
    faceMeasure_ii = FACEMEASURE{iface};
            ax_idx = iface;
            for kk = Nrad
                FC_current = prctile(squeeze(FC_cum_val_all(:,iface,kk,:,:)),50,2);
                inside_logic_current = logical(squeeze(inside_logic_all(:,iface,kk,:,:)));
                inside_logic_last_current = find(mean(inside_logic_current,2)>0.95,1);
                plot(h_ax(ax_idx),nf_all(:,1),FC_current,'-','Color',color_set_cell{kk},'LineWidth',2);
%                 plot(h_ax(ax_idx),nf_all(inside_logic_last_current,1),FC_current(inside_logic_last_current),'r.','MarkerSize',30);
            end
            
            FC_current = squeeze(FC_cum_val_in_annulus_all(:,iface,:));
            plot(h_ax(ax_idx),nf_all(:,1),FC_current,'-.','Color','k','LineWidth',lw);
            FC_current = squeeze(FC_cum_val_out_annulus_all(:,iface,:));
            plot(h_ax(ax_idx),nf_all(:,1),FC_current,'-','Color','k','LineWidth',lw);
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
            if (iface==1); ylabel(h_ax(ax_idx),'Rate (%/s)','FontSize',25); end
            %
            grid(h_ax(ax_idx),'on'); 
end
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Radial time %s-%s-%s-%s-rate",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.4,0.0287,0.2,0.8926]);
Nface = length(FACEMEASURE);
Nrow = Ncirc; 
Ncol = length(FACEMEASURE); 
mergeElement = {};
...
for iface = 1:Nface
    ...
    for xx = 1:Ncirc
        for kk = 1:Nrad-1
            ax_idx = iface+(xx-1)*Ncol;
            inside_logic_current = logical(squeeze(inside_logic_all(:,iface,kk,xx,:)));
            FC_current = squeeze(FC_rate_val_all(:,iface,kk,xx,:));
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
%                 'nstd',nstd,...
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            plot(h_ax(ax_idx),t,FC_current,'Color',color_set_cell{kk},'LineWidth',lw);
            % x-axis
            xlim(h_ax(ax_idx),[0,t(end)]);
            if xx~=Ncirc; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if xx~=Ncirc; h_ax(ax_idx).XTickLabel = []; end
            if xx==Ncirc; xlabel(h_ax(ax_idx),'Time (s)'); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylimrate); end
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
            if (iface==1) && (xx==(floor(Ncirc/2)+1)); ylabel(h_ax(ax_idx),'RATE (%/s)','FontSize',25); end
            %
            if (iface==2)&&kk==Nrad
                text(h_ax(ax_idx),-1,10,T{1}.name_circ{xx},'FontSize',12,'Interpreter','none',...
                'HorizontalAlignment','center');
            end
            %
            grid(h_ax(ax_idx),'on'); 
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Circumferencial time %s-%s-%s-%s-rate",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.6,0.0287,0.2,0.8926]);
Nface = length(FACEMEASURE);
Nrow = Nrad; 
Ncol = length(FACEMEASURE); 
mergeElement = {};
...
for iface = 1:Nface
    ...
    for kk = 1:Nrad
        for xx = 1:Ncirc
            ax_idx = iface+(kk-1)*Ncol;
            inside_logic_current = squeeze(inside_logic_all(:,iface,kk,xx,:)); 
            FC_current = squeeze(FC_rate_val_all(:,iface,kk,xx,:));
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            plot(h_ax(ax_idx),t,FC_current,'Color',color_set_cell{xx},'LineWidth',lw);
            % x-axis
            xlim(h_ax(ax_idx),[0,t(end)]);
            if kk~=Nrad; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if kk~=Nrad; h_ax(ax_idx).XTickLabel = []; end
            if kk==Nrad; xlabel(h_ax(ax_idx),'Time (s)'); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylimrate); end
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
            if (iface==1) && (kk==(floor(Nrad/2)+1)); ylabel(h_ax(ax_idx),'RATE (%/s)','FontSize',25); end
            %
            if (iface==2)&&xx==Ncirc
                text(h_ax(ax_idx),-1,10,T{1}.name_rad{kk},'FontSize',12,'Interpreter','none',...
                'HorizontalAlignment','center');
            end
            %
            grid(h_ax(ax_idx),'on'); 
...
% Figure and subplot initialization 
title_fig = sprintf("%s : Annulus time %s-%s-%s-%s-rate",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);

set(hf,'Color','White','Unit','Normalized',...
    'Position',[0.0005    0.6963    0.3870    0.2250]);
Nface = length(FACEMEASURE);
Nrow = 1; 
Ncol = length(FACEMEASURE); 
mergeElement = {};
...
for iface = 1:Nface
    faceMeasure_ii = FACEMEASURE{iface};
            ax_idx = iface;
            FC_current = squeeze(FC_rate_val_out_annulus_all(:,iface,:));
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
%                 'h_ax',h_ax(ax_idx),...
%                 'nstd',nstd,...
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            plot(h_ax(ax_idx),t,FC_current,'Color','k','LineWidth',lw);
            % x-axis
            xlim(h_ax(ax_idx),[0,t(end)]);
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            plot(h_ax(ax_idx),nf_all,FC_current,'Color',color_set_cell{kk},'LineWidth',lw);
            % x-axis
            xlim(h_ax(ax_idx),[0,5]);
            if xx~=Ncirc; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if xx~=Ncirc; h_ax(ax_idx).XTickLabel = []; end
            if xx==Ncirc; xlabel(h_ax(ax_idx),'Force (N)'); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylimrate); end
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
...
for iface = 1:Nface
    ...
    for kk = 1:Nrad
        for xx = 1:Ncirc
            ...
            FC_current = squeeze(FC_rate_val_all(:,iface,kk,xx,:));
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{xx},...
%                 'h_ax',h_ax(ax_idx),...
%                 'nstd',nstd,...
%                 'FaceAlpha',0.5);
%             plot(h_ax(ax_idx),nf_all,mean(FC_current,2),'Color',color_set_cell{xx},'LineWidth',lw);
            
            plot(h_ax(ax_idx),nf_all,FC_current,'Color',color_set_cell{xx},'LineWidth',lw);
            ...
            if kk~=Nrad; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if kk~=Nrad; h_ax(ax_idx).XTickLabel = []; end
            if kk==Nrad; xlabel(h_ax(ax_idx),'Force (N)'); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylimrate); end
            if (iface~=1); set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if iface~=1; h_ax(ax_idx).YTickLabel = []; end
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            plot(h_ax(ax_idx),nf_all,FC_current,'Color','k','LineWidth',lw);
            
            FC_current = squeeze(FC_rate_val_in_annulus_all(:,iface,:));
            plot(h_ax(ax_idx),nf_all,FC_current,'Color','k','LineWidth',lw);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% FIGURE TOTAL with displacement
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ms = 4; 
marker_set = 'o'; 
xlabel_set = 'Displacement (mm)'; xlim_set = [0,4];
ylabel_set = 'TOTAL (%)'; ylim_set = [-10e-2,10e-2];
...
for iface = 1:2
    ...
    for xx = 1:Ncirc
        ...
        for kk = 1:Nrad-1
            ...
            FC_current = squeeze(FC_cum_val_all(:,iface,kk,xx,:));
            if kk == 1
                t_touch = zeros(size(inside_logic_current,2),1);
                for mm = 1:size(inside_logic_current,2)
                    t_touch(mm) = idx_touch_all(mm);%find(inside_logic_current(:,mm),1);
                end
            end
            if ~isempty(t_touch)
                xstart = diag(squeeze(FC_cum_val_all(t_touch,3,kk,xx,:)))';
                x = squeeze(FC_cum_val_all(:,3,kk,xx,:))-xstart;
            end
            inside_logic_current_mean = mean(inside_logic_current,2)>0.5;
%             shaded_std_plot(t,FC_current,...
%                 'Color',color_set_cell{kk},...
...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor','k')
            ...
            masktrial = 1:Ntrial/2; 
            %Line plot
            plot(h_ax(ax_idx),x(:,masktrial),FC_current(:,masktrial),'Color',[1 1 1].*0.5,...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
%             %Colored dots - inside contact
%             plot(h_ax(ax_idx),x(inside_logic_current(:,masktrial)),FC_current(inside_logic_current(:,masktrial)),'Color',[1 1 1].*0.5,...
%                 'LineStyle','none',...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',[1 1 1].*0.5);
            
            masktrial = Ntrial/2+1:Ntrial; 
            %Line plot
            plot(h_ax(ax_idx),x(:,masktrial),FC_current(:,masktrial),'Color',[1 1 1].*0.0,...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
%             %Colored dots - inside contact
%             plot(h_ax(ax_idx),x(inside_logic_current(:,masktrial)),FC_current(inside_logic_current(:,masktrial)),'Color',[1 1 1].*0.0,...
%                 'LineStyle','none',...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',[1 1 1].*0.0);
...
set(h_ax,'FontSize',12);%,'Clipping','off');
%%
newfig('displacement and force, touch instant'); 
...
for mm = 1:Ntrial 
    ...
    idx_cur = idx_touch_all(mm); 
    nf_cur = T{mm}.nf; 
    displ = squeeze(FC_cum_val_all(:,4,1,1,mm));
    plot(ax1,t-t(idx_cur),nf_cur,'Color',c); 
    plot(ax2,t,displ,'Color',c);
    plot(ax3,nf_cur,displ,'Color',c); 
    plot(ax3,nf_cur(idx_cur),displ(idx_cur),'rx');
    plot(ax4,nf_cur-nf_cur(idx_cur),displ-displ(idx_cur),'Color',c);
end
%%
%--------------------------------------------------------------------------
% Segmentation CIRCUMPHERENCIAL 
%--------------------------------------------------------------------------
% settings : colormap, legend name,...
xlabel_set = 'Time (s)'; xlim_set = [0,3];
ylabel_set = 'Displacement (mm)'; ylim_set = [0,600];
C = colororder(autumn(Ncirc)); %parula
color_set_array = C(1:Ncirc,:);
lgd_name = T{1}.name_circ;
color_set_cell = mat2cell(color_set_array,repelem(1,size(color_set_array,1)),[3]);
...
% Figure and subplot initialization 
title_fig = sprintf("%s : time vs displ %s-%s-%s-%s-cum",SUBJECT,'speckles',...
    material,PHASE); 
hf = newfig(title_fig);
...
for iface = 1
    ...
    for kk = Nrad-1
        ...
        for xx = 1:Ncirc
            inside_logic_current = logical(squeeze(inside_logic_all(:,iface,kk,xx,:))); 
            FC_current = squeeze(FC_cum_val_all(:,4,kk,xx,:));
            x = repmat(t,Ntrial,1)';
...
%             plot(h_ax(ax_idx),t,mean(FC_current,2),'Color',color_set_cell{kk},'LineWidth',lw);
            ...
            plot(h_ax(ax_idx),x(:,masktrial),FC_current(:,masktrial),'Color',[1 1 1].*0.5,...
                ...
                'Marker','none');
            masktrial = Ntrial/2+1:Ntrial; 
            plot(h_ax(ax_idx),x(:,masktrial),FC_current(:,masktrial),'Color','k',...
                'LineWidth',lw,'LineStyle','-',...
                'Marker','none');
%             %Black dots - outside contact
%             plot(h_ax(ax_idx),x(~inside_logic_current),FC_current(~inside_logic_current),'Color',color_set_cell{xx},...
...
%                 'Marker',marker_set,'MarkerSize',ms,'MarkerFaceColor',color_set_cell{xx});
            %inside logic 
            if xx==round(Ncirc/2)
                x = nf_all(mean(~inside_logic_current,2)>0.5,2); x = [xlim_set(1),max(x)];
                fill(h_ax(ax_idx),[x fliplr(x)],repelem(ylim_set,1,2),[1 1 1].*0.0,'LineStyle','none','FaceAlpha',0.1);
            end
            %Title
            if (ax_idx <= Ncol); title(h_ax(ax_idx),sprintf('%s',faceMeasure_ii),'fontweight','bold','fontsize',25);  end
            % x-axis
            xlim(h_ax(ax_idx),xlim_set);
            if false; set(get(h_ax(ax_idx), 'XAxis'), 'Visible', 'off'); end
            if false; h_ax(ax_idx).XTickLabel = []; end
            if true; xlabel(h_ax(ax_idx),xlabel_set); end
            % y-axis
            if strcmp(faceMeasure_ii,'Epc1')||strcmp(faceMeasure_ii,'Epc2'); ylim(h_ax(ax_idx),ylim_set); end
            if false; set(get(h_ax(ax_idx), 'YAxis'), 'Visible', 'off'); end
            if false; h_ax(ax_idx).YTickLabel = []; end
            ylabel(h_ax(ax_idx),ylabel_set,'FontSize',25);
            %
            if xx==Ncirc
                text(h_ax(ax_idx),mean(xlim_set),ylim_set(2),T{1}.name_rad{kk},...
                    'FontSize',12,'Interpreter','none',...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% RADIAL box plot fig : within segmentation, across ROI  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lgd_name = T{1}.name_rad;
ylim_set = [-15,15];
disp('done');
% figure settings
title_fig = sprintf("%s : Radial total final %s-%s-%s-%s-%dN-%1.2f-cum",SUBJECT,'speckles',...
    material,PHASE,NFCOND_SET,SPDDXLCOND_SET); 
hf = newfig(title_fig);
set(hf,'Unit', 'Normalized','Position',[0   0.03    0.32    0.8926]);
set( hf, 'Color', 'White') ;
color_line_intersection = {'b','r'};  
% ax size
Nrow = 1; %Ncirc; 
...
for mm = 1:Ntrial
    ...
    ax_idx = mm; %+(xx-1)*Ncol;
    for xx = 1:Ncirc
        FCcurrent = [FCcurrent;squeeze(FCmat_cum_last_all{mm}(:,:,xx))];
    end
    ...
    x = mat2cell(FCcurrent,Ndata,repelem(Nrad,Nface)); 
    h = boxplotGroup(h_ax(ax_idx),x,'groupLines',true,...
        'PrimaryLabels',FACEMEASURE(1:Nface), ...
        'SecondaryLabels',lgd_name(1:Nrad), ...
...
%     Nframe = T{mm}.Nframe; 
    ...
    idx_rad_target = idx_frontal_target; %T{mm}.mask_circ{2}(:,1)|T{mm}.mask_circ{3}(:,1)|T{mm}.mask_circ{4}(:,1); 
    ...
    for irad = 1:Nrad_in    
        ...
        idx_IN_rad_raw{irad} = idx_IN_current & ~idx_IN_allprevious & idx_frontal_target;% & idx_rad_target;
    end
    Nrad_border = 1;
    ...
    %Merge (#margin) number of radial segmentation at the origin
    idx_IN_rad_first = false(length(PCtarget),1); 
    for irad = 1:rad_combined_number
...
for mm = 1:Ntrial
    ...
    idx_nf = t(find(nf_all(:,mm)>mag_nf,1,'first')); 
    ...
    ax_idx = 2; 
    yline(h_ax(ax_idx),0,'k'); 
    plot(h_ax(ax_idx),nf_all(:,mm),squeeze(FCmat_annulus(:,1)),'Color',[1 1 1].*0.5,'LineWidth',2);
    plot(h_ax(ax_idx),nf_all(:,mm),squeeze(FCmat_annulus(:,2)),'Color',[1 1 1].*0.0,'LineWidth',2);
...
list_force_target = [0.0,0.02,0.05,0.10,0.20,0.50,1:1:5];
ylim_set = [-40,40];

color_face = [[1 1 1].*0.7;[1 1 1].*0.0]; %[1 1 1].*(linspace(0,0.7,Ntrial))';
fig_name = sprintf('%s - Epc1,Epc2 front - rate multiple rad all',SUBJECT);%%1.3f,list_force_target); 
fig = newfig(fig_name); 
set(fig,'Position',[3.7306   10.1865   13.4144   13.9435]); 
...
for mm = 1%1:Ntrial
    ...
    list_frame_num = find_closest_value(nf_format,list_force_target)';%1:150;
...
for mm = 1:Ntrial
    ...
    plot(h_ax(1),xvec,val(mm,:)','-','LineWidth',1,'Color',color_face(iface,:)); %[1 1 1].*color_scale_grey(ii));

end 
plot(h_ax(1),xvec,mean(val)','-','LineWidth',3,'Color',color_face(iface,:)); %[1 1 1].*color_scale_grey(ii));
end
end
exportgraphics(fig,sprintf('fig/%s.%s',fig_name,'png')); 
...
% C = colormap(gray(round(Nrad_segmentation*(1.2)))); 
frame_target = 1; 
list_face_target_num = [4];
list_frame_num = 27:5:97;
...
%     FCmat_annulus = FC_cum_val_out_annulus_all(:,:,mm);
    ...
    set(h_ax(ax_idx),'XLim',[0,xvec_nf(end)+0.5],'YLim',[0,2e-3],'Clipping','off');%[-0.025,0.025][-1.5,1.5]
    ylim_set = get(h_ax(ax_idx),'YLim');
%     for kk = 1:Nrad_segmentation
%         plot(h_ax(ax_idx),[kk,kk],[ylim_set(1),FCmat_cur(end,kk)],'k-','LineWidth',1); 
%     end
...
for mm = 1%:Ntrial
    ...
    for irad = list_rad_num
        ...
        for iface = list_face_target_num
            ...
            %face measure ii
            hp{kk} = plot(h_ax(ax_idx),nf_all(:,mm),FCmat_curv_current,'-','LineWidth',1,'Color',color_face(kk,:)); 
%             scatter(h_ax(ax_idx),nf_all(:,mm),FCmat_curv_cur_in,30,FCmat_rate{mm}(list_frame_num,irad+(iface-1)*Nrad_segmentation),'filled');
%             scatter(h_ax(ax_idx),nf_all(:,mm),FCmat_curv_cur_out,30,FCmat_rate{mm}(list_frame_num,irad+(iface-1)*Nrad_segmentation),'Marker','s','MarkerFaceColor','w','LineWidth',2);
            scatter(h_ax(ax_idx),nf_all(:,mm),FCmat_curv_current,30,FCmat_rate{mm}(list_frame_num,irad+(iface-1)*Nrad_segmentation),'filled');

            h_ax(ax_idx).XLim = [0,2];%NFCOND_SET]; 
            h_ax(ax_idx).YLim(1) = 0;%,3e-3];
...
list_force_target = [0.02,0.03,0.04,0.05,0.10,0.20,0.50,1:1:5];
A = zeros(1,Nframe); 

fig_name = sprintf('Contact Area Evolution %s',SUBJECT);%,list_force_target);  %1.3fN
fig = newfig(fig_name); 
set(fig,'Unit','Normalized','Position',[0.1208    0.0287    0.1672    0.8926]); 
Nrow = 3; Ncol = 1; 
h_ax = subplot_ax(Nrow,Ncol); hold(h_ax,'on'); 
for mm = 1:Ntrial
    for iframe = 1:Nframe
...
%     h_ax(ax_idx).YLabel.String = 'Indentation [mm]';
    ax_idx = 1; 
    plot(h_ax(ax_idx),t,nf_all(:,mm),'k.-','MarkerSize',8);
    ...
    ax_idx = 3; 
    plot(h_ax(ax_idx),nf_all(:,mm),A,'k.-','MarkerSize',8);
...
%     h_ax(ax_idx).YLabel.String = 'Area [mm2]';
%     ax_idx = 2; 
%     plot(h_ax(ax_idx),displ,nf_all(:,mm),'k.-','MarkerSize',8);
...
mvaluecf1 = NaN(Ntrial,1);
mvaluestrain = NaN(Ntrial,length(face_target));
...
%             idx = idx_1N_force;
        ...
        seg_target = 1:Nrad_in;
        FCmat_cur = FCmat_cum{mm};
        idx_1N_force = find(nf_format>NFCOND_SET*0.95,1,'first');
        mvaluestrain(mm,iface) = mean(FCmat_cur(idx_1N_force,seg_target+(iface-1)*Nrad_segmentation));
        ax_idx = 2+iface; 
        plot(h_ax(ax_idx),mvaluecf1(mm),mvaluestrain(mm,iface),'.','Color',C(mm,:));
